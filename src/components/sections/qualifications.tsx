import { QUALIFICATIONS, SkillNames, SKILLS } from "@/data/constants";
        ...
        <div className="flex flex-col gap-8 md:gap-12 relative">
          ...
          {QUALIFICATIONS.map((qual, index) => (
            <div key={qual.id} className="relative">
              <QualificationCard qualification={qual} index={index} />
            </div>
          ))}
        </div>
      </div>
      ...
      >
        ...
        <CardContent className="space-y-6">
          <ul className="list-disc list-outside ml-4 space-y-2 text-base text-muted-foreground leading-relaxed">
            {qualification.description.map((point: string, i: number) => (
              <li key={i}>{point}</li>
            ))}
          </ul>

          {qualification.skills.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {qualification.skills.map((skillName: SkillNames) => {
